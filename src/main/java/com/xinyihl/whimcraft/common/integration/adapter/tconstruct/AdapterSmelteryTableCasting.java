...
public class AdapterSmelteryTableCasting extends RecipeAdapter {
    public AdapterSmelteryTableCasting() {
        super(new ResourceLocation("mctsmelteryio", "whimcraft_smeltery_table_casting"));
    }
    ...
    public Collection<MachineRecipe> createRecipesFor(ResourceLocation owningMachineName, List<RecipeModifier> modifiers, List<ComponentRequirement<?, ?>> additionalRequirements, Map<Class<?>, List<IEventHandler<RecipeEvent>>> eventHandlers, List<String> recipeTooltips) {
        ...
        TinkerRegistry.getAllTableCastingRecipes().forEach((irecipe -> {
            ...
            MachineRecipe machineRecipe = createRecipeShell(
                    new ResourceLocation("mctsmelteryio", "whimcraft_auto_smeltery_table_casting" + incId),
                    owningMachineName,
                    recipe.getTime(),
                    incId, false);
