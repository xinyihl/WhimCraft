...
import com.xinyihl.whimcraft.common.inventory.InfinityListGasCellInventory;
...
public class InfinityListGasCellHandler implements IInfinityListCellHandler {
    ...
    public boolean isCell(ItemStack itemStack) {
        return itemStack.getItem() instanceof InfinityListGasCell;
    }
    ...
    public <T extends IAEStack<T>> ICellInventoryHandler<T> getCellInventory(ItemStack itemStack, ISaveProvider iSaveProvider, IStorageChannel<T> iStorageChannel) {
        if (itemStack.getItem() instanceof InfinityListGasCell && iStorageChannel == AEApi.instance().storage().getStorageChannel(IGasStorageChannel.class)) {
            return new BasicCellInventoryHandler(new InfinityListGasCellInventory(itemStack), iStorageChannel);
        }
        return null;
    }
